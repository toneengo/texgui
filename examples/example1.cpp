...
int main() {
  ...
  unsigned char *im =
      stbi_load("resources/windows.jpg", &width, &height, &channels, 4);

  GLuint bg;
  glCreateTextures(GL_TEXTURE_2D, 1, &bg);

  glTextureStorage2D(bg, 1, GL_RGBA8, width, height);
  glTextureSubImage2D(bg, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE,
                      im);
...
void main() {
  ...
  glAttachShader(bg_shader, vsh);
  glAttachShader(bg_shader, fsh);
  ...
  int val;
  for (int i = 0; i < 15; i++) {
    Widget *item1 = listrow->addCol(
        new ListItem(new Image("lollipop"), &val, Lollipop), -1);
    Widget *item2 =
        listrow->addCol(new ListItem(new Image("tennis"), &val, Tennis), -1);
  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    Button imm(
        "press", []() { printf("im drawn every frame\n"); }, 200, 200, 500,
        200);
    imm.draw(state);
    glctx->render(state);
